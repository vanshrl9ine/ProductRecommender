...
if choice == 'Sign up':
    ...
    if user_location:
        ...
        if location:
            ...
            db.child("user_locations").push(data)
    ...
    def func():
        ...
        if st.button('Recommend'):
            if selected_pid:
                ...
                db.child("user_searches").push(search_data)
            ...
            for i,x in enumerate(recommendations.tolist()):
                st.write(f'{x}')
                pid = get_product_pid(x)
                imageobj = get_product_image(x)
                if imageobj is not None:
                    st.image(imageobj, width=300)

                added_bool= st.button(f'Add {pid} to Cart', key=f'add_to_cart_{i}')

                if(added_bool):
                    added_to_cart.append(pid)
            for item in added_to_cart:
                cart_pid=get_product_pid(item)
                cart_tags_data = tags_from_pid(selected_pid)
                cart_data={
                    "cart_pid": cart_pid,
                    "cart_tags":cart_tags_data

                }
                db.child("user_cart").push(cart_data)

            for pid in added_to_cart:
                tagsdata = tags_from_pid(pid)
                print(tagsdata)#debugging statement not printing
                cartdata = {
                    "product": pid,  # Use pid instead of x (product title)
                    "tags": tagsdata
                }
                st.session.state.cart.append(cartdata)
                db.child("user_tags").push(cartdata)
